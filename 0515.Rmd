...
```{r}
factor1 <- factor(c("新北市","台北市","台中市","彰化市","基隆市","苗栗縣"))
factor1
```
...
library(stringr)
garbageRoutes$route <- 
  factor(str_extract(garbageRoutes$linename, "[:alnum:]+路線"))
levels(garbageRoutes$route)
```

```{r}
newLevels <- names(sort(table(garbageRoutes$route)))
garbageRoutes$route <- factor(garbageRoutes$route, newLevels)
levels(garbageRoutes$route)
```
